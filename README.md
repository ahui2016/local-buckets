...
> 資料: 是指文檔, 圖片等各種檔案, 以下統稱爲 "檔案".

## 主要功能

- 附加属性
  - 每个档案除了拥有名称, 大小, 日期等基本属性之外, 还额外拥有:
  - Notes (备注, 用来进一步描述档案)
  - Keywords (关键词, 用来帮助找到档案)
  - Like (小心心❤, 用来标记特殊档案, 比如特别喜爱, 特别重要, 特别常用等等)
  - Checksum (校验和, 用来判断档案是否受损)
- 备份
  - 提供方便的备份功能, 方便备份到移动硬盘中.
  - 采用单向备份方式, 逻辑极致简单, 不容易出错, 操作也极致方便 (完全不需要处理冲突).
  - 可检查档案完整性, 一键修复受损档案.
- 加密
  - 可创建 **加密仓库**, 添加到加密仓库中的档案会被自动加密
...
- 搜尋後, 按 F5 或 Ctrl+R 可返回完整檔案清單 (最近檔案清單).

## 小心心❤

點擊 `info` 按鈕打開側邊欄, 可編輯檔案屬性,
其中, 可設置 **like** 的數量, 也就是小心心❤的數量.
